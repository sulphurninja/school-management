...
export default function TeacherSchedule() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Failed to add timetable");
      ...
      console.error(err);
      toast({ title: "Error", description: "Could not add timetable", variant: "destructive" });
    }
  };
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 mt-4">
            {["classId", "section", "startTime", "endTime", "subject", "room"].map((field) => (
              <div key={field}>
                <Label>{field.charAt(0).toUpperCase() + field.slice(1)}</Label>
                <Input name={field} value={(formData as any)[field]} onChange={handleInputChange} />
              </div>
            ))}
            <div>
              <Label>Day</Label>
              <select name="day" value={formData.day} onChange={handleInputChange} className="w-full border rounded px-2 py-1">
